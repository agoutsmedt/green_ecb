...
chosen_topic_model <- readRDS(here(data_path, "topic_modelling", "TM_data_set_inflation.rds")) %>% 
  filter(preprocessing_id == 1)

doc_paragraphs <- chosen_topic_model$data[[1]] %>% 
...
metadata <- data.table("document" = rownames(chosen_topic_model$dfm[[1]])) %>% 
  left_join(doc_paragraphs) %>% 
  mutate(date = dmy(date),
         quarter = quarter(date),
         semester = semester(date))

metadata_with_text <- metadata %>% 
  left_join(rename(eurosystem_text, "document" = document_id)) %>% 
  arrange(document)
...
ggsave(here("pictures", glue::glue("TM_topic_correlation.png")),
       graph_plot,
       device = ragg::agg_png,
...
topic_examples <- topics_complete %>% 
  ...
  mutate(examples = map(id, ~findThoughts(topic_model, 
                                          texts = paste0(metadata_with_text$title, " (",
                                                         metadata_with_text$document, ") - ",
                                                         metadata_with_text$central_bank, " (",
                                                         metadata_with_text$pdf_link, "):",
                                                         metadata_with_text$paragraphs),
                                          n = 4, 
                                          topics = .x)$docs[[1]])) %>% 
  unnest(examples)

test <- findThoughts(topic_model, 
                     texts = metadata_with_text$paragraphs,
                     n = 4, 
                     topics = 16)
...
tibble(V1 = names(data_set$stm[[1]]$document),
       V2 = rownames(data_set$dfm[[1]]),
       V3 = names(chosen_topic_model$stm[[1]]$document),
       V4 = metadata_with_text$document) %>% 
  mutate(V1_V3 = V1 == V3,
         V1_V2 = V1 == V2,
         V1_V4 = V1 == V4) %>% View()

dt <- make.dt(topic_model, meta = data_set$stm[[i]]$meta)
texts <- dt[, docnum[order(Topic24, decreasing=TRUE)][1:5]]
dt[texts]
