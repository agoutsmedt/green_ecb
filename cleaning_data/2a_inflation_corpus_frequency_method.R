...
stats_inflation_texts <- eurosystem_metadata %>% 
  ...
  mutate(small_threshold = sum_inflation_words >= 5,
         ...
         big_threshold = sum_inflation_words >= 20) %>% 
...
eurosystem_text %>% 
  filter(file %in% filter(inflation_text, sum_inflation_words >= 20)$file) %>% 
  saveRDS(here(data_path, "eurosystem_text_inflation.rds"))
